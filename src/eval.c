...
#	ifdef TUNE
	...
	evalOptionNewVPair("KnightPawnAffinity", &evalKnightPawnAffinity, -300, 300);
	evalOptionNewVPair("BishopPair", &evalBishopPair, 0, 1000);
	evalOptionNewVPair("BishopMobility", &evalBishopMob, 0, 300);
	...
	evalOptionNewVPair("RookMobilityFile", &evalRookMobFile, 0, 200);
	evalOptionNewVPair("RookMobilityRank", &evalRookMobRank, 0, 200);
	evalOptionNewVPair("RookOpenFile", &evalRookOpenFile, -1000, 1000);
	evalOptionNewVPair("RookSemiOpenFile", &evalRookSemiOpenFile, 0, 500);
	...
	evalOptionNewVPair("KingShieldFar", &evalKingShieldFar, -300, 300);
	evalOptionNewVPair("KingCastlingMobility", &evalKingCastlingMobility, -200, 200);
	evalOptionNewVPair("OutpostSq", &evalOutpostSq, -200, 500);
...
VPair evaluateDefault(EvalData *data) {
	...
	// Bishop mobility
	BB bishopMobOcc[ColourNB];
	bishopMobOcc[ColourWhite]=(posGetBBAll(pos)^(wb|wq));
	bishopMobOcc[ColourBlack]=(posGetBBAll(pos)^(bb|bq));
	...
	// Rook mobilty
	BB rookMobOcc[ColourNB];
	rookMobOcc[ColourWhite]=(posGetBBAll(pos)^(wr|wq));
	rookMobOcc[ColourBlack]=(posGetBBAll(pos)^(br|bq));
	...
	while(pieceSet) {
		Sq sq=bbScanReset(&pieceSet);
		BB attacks=(attacksBishop(sq, bishopMobOcc[ColourWhite])|attacksRook(sq, rookMobOcc[ColourWhite]));
		attackArray[PieceWQueen]|=attacks;
		evalVPairAddMulTo(&score, &evalBishopMob, bbPopCount(attacks & mobilityAllowed[ColourWhite]));
	}
	pieceSet=bq;
	while(pieceSet) {
		Sq sq=bbScanReset(&pieceSet);
		BB attacks=(attacksBishop(sq, bishopMobOcc[ColourBlack])|attacksRook(sq, rookMobOcc[ColourBlack]));
		attackArray[PieceBQueen]|=attacks;
		evalVPairSubMulFrom(&score, &evalBishopMob, bbPopCount(attacks & mobilityAllowed[ColourBlack]));
	}
...
void evalComputePawnData(const Pos *pos, EvalPawnData *pawnData) {
	...
	// Doubled, isolated and connected pawns
	int doubledCount=((int)bbPopCount(doubled[ColourWhite]))-((int)bbPopCount(doubled[ColourBlack]));
