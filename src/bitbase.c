...
STATICASSERT(BitBaseResultDraw<2 && BitBaseResultWin<2);
...
void bitbaseGen(void)
{
  ...
  for(pawnRank=Rank7;pawnRank>=Rank2;--pawnRank)
  {
    ...
    // Update global array.
    // Any positions left 'unknown' are draws (although neither side can 'force'
    // it per se).
    for(wKingSq=0;wKingSq<SqNB;++wKingSq)
    for(stm=0;stm<ColourNB;++stm)
    {
...
BitBaseResultFull bitbaseComputeDynamicResult(const BitBaseResultFull *array, Sq pawnSq, Sq wKingSq, Colour stm, Sq bKingSq)
{
  ...
  if (stm==ColourWhite)
  {
    ...
    unsigned int newIndex=bitbaseIndexFull(pawnFile, pawnRank+1, wKingSq, colourSwap(stm), bKingSq);
