...
void posSetToArray(Pos *pos, Colour stm, Piece array[SqNB]); // note: only the bitboards and stm are set so the resulting Pos struct will almost certainly not pass a posIsConsistent check (we also skip NNUE accumulator updates)
